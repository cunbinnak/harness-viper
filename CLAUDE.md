...
`guard_ask` (AskUserQuestion chỉ ở DOCUMENT; Scope khoá ✓ / ngoài DOCUMENT → chặn) · `guard_bc` (deploy khi BC §3 xanh) · `guard_ds` (mockup dùng token, không hex thô) · `guard_archive` (cấm sửa `archive/**`) · `guard_proof` (cấm sửa `*proof.json`) · `guard_makefile` (cấm sửa ROOT Makefile — điền per-target) · `guard_doc` (doc SPEC đóng băng ngoài DOCUMENT → sửa = wave sau) · `reanchor` (nhồi lại luật sau compact). `gate.py` chỉ BÁO.
