...
Trước khi báo xong (cuối VERIFY; và trên production nếu wave khai SHIP): chạy `/dogfood` — MAIN **tự tay dùng TRƯỚC** bằng trình duyệt thật (skill `browse`, tool `browser_*` Playwright), vào từ trang đầu đi hết luồng lõi, đối chiếu từng màn với mockup đã chốt; RỒI mới 6 persona × 2 đợt (≤3 vai/đợt, seed lại giữa đợt). **curl KHÔNG PHẢI dogfood** (chỉ hợp lệ khi target `KHÔNG CÓ UI`). `picky` là lớp canh design-system + mockup **DUY NHẤT** sau DOCUMENT (`guard_ds`/`guard_shell` chỉ soi mockup HTML, không soi code render) — bỏ picky = không gì bắt UI lệch bản chốt. Duyệt web trong dự án này CHỈ bằng skill `browse` — kể cả khi cấu hình toàn cục chỉ định công cụ khác.
