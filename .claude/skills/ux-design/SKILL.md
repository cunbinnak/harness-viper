...
Đừng tự vẽ từ đầu thứ đã có người làm đẹp sẵn. Trước khi vẽ tay, **tìm template/UI-kit mã nguồn mở hợp domain** (admin dashboard: Tabler/TailAdmin/AdminLTE · landing/app: HTML template MIT trên GitHub) và **LẤY LUÔN làm nền mockup**:
1. **Chọn** template license cho phép (MIT/Apache/CC — ghi **tên + URL + license** vào `DESIGN-SYSTEM §1` làm neo).
