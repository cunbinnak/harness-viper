---
name: ux-design
description: UX/UI cho web target (bước 8 mockup của /document) — user flow, MOCKUP HTML tĩnh 1 file/MODULE, mỗi màn + tab/modal/form là section có mã, bấm được (design bằng HTML, không ASCII), UI states đầy đủ, design tokens + Visual polish, a11y WCAG 2.1 AA, permission-based UI. Sinh docs/ux/SCREEN-MAP.md (Mã màn nối mockup↔FE↔picky) + docs/ux/mockups/{name}/<module>.html; token ở docs/DESIGN-SYSTEM.md §2.
---
...
## Deliverable
**Đơn vị thiết kế = MÀN (screen); đơn vị FILE = MODULE nghiệp vụ** — mỗi module (vd `employee`: danh sách · chi tiết · form · tab · modal) = **1 file mockup chứa đủ màn + mọi tương tác**, bấm được như prototype. Ít file để đọc, luồng trong module liền mạch, lười màn con lộ ngay (bấm nút không đi đâu). Boundary chỉ là nơi màn thuộc về. Sản phẩm gồm:
- **`docs/ux/SCREEN-MAP.md`** — MỤC LỤC MÀN, sinh TRƯỚC khi vẽ, **đúng format `templates/TEMPLATE.screen-map.md`**: `| Mã màn | Module | Tên màn | Target | URL mẫu | Mockup | Mở từ | Wave | FEAT:AC | Ghi chú |` — mỗi màn **và mỗi màn con (tab/modal/form)** 1 row. **Mã màn = MỘT mã nối tất cả**: `id` section trong mockup = `data-screen` mockup = `data-screen` trang FE (màn con `<MÃ-CHA>--<tên>`). **URL mẫu** mở được ngay (id từ dữ liệu mẫu PRD §7, không để `:id`). Ô **Mockup CHỈ 2 dạng**: `<module>.html#<Mã màn>` hoặc `(Wave N)`. **Mở từ** = thao tác tới màn (từ màn nào, bấm gì) — picky làm y hệt hai bên. Gate đỏ nếu row in-scope trỏ file/section không tồn tại. Cột **Ghi chú** ghi behavior ngắn (empty→CTA, list realtime, validation…). **Luật gán màn → target** (quan trọng khi NHIỀU web target): (1) FEAT frontmatter `target_hint`; (2) không có hint → journey + persona của FEAT so với persona pool của target (`PERSONAS.md` + `CAPABILITIES-MAP.md`); (3) vẫn mơ hồ → **hỏi user**, KHÔNG đoán. Mỗi FEAT `has_ui_touchpoint=true` phải có ≥1 màn; màn dùng chung nhiều FEAT (list/detail) = 1 row ghi nhiều FEAT.
...
- [ ] Mọi row SCREEN-MAP in-scope trỏ `<module>.html#<Mã màn>` **tồn tại** (file + section `id`) — mở `file://…#Mã` tới thẳng; mọi thao tác AC có link/nút dẫn tới section; responsive; đủ section state phụ.
