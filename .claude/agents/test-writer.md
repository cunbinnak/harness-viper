...
- **Thiếu TOOL cũng ≠ không test được.** Tool cài được trong env (Playwright `npx playwright` · k6 · autocannon) → **cài rồi chạy**, không phải cớ skip. AC khai ngưỡng đo được (vd ≤300ms) = NFR **trong scope** — không có k6 vẫn đo thô được (`curl -w '%{time_total}'` loop) đủ mức chấm ngưỡng. **"CI không có" là lạc đề** — VERIFY chạy LOCAL trên hệ thật. **CẤM defer TC của AC in-scope sang "wave sau" tự chế** — defer chỉ hợp lệ khi ROADMAP/Authority khai NFR đó ngoài wave, và defer = TC ghi `chưa chạy — defer wave N` + 1 dòng `ROADMAP §backlog`, AC đó **KHÔNG tính PASS** (gate sẽ đỏ, đúng thiết kế).
